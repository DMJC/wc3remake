...
class WC3Globals {
public:
    static WC3Globals& getInstance() {
        static WC3Globals inst;
        return inst;
    }
    ...
    WC3Font* getFont(const char* name) const;
...
private:
    WC3Globals() = default;
    WC3Globals(const WC3Globals&) = delete;

    bool loaded{false};
    float tickRate{120.0f};

    std::unordered_map<std::string, std::string> paths;
    std::unordered_map<std::string, WC3Font*>    fonts;
