...
uint32_t WC4Globals::r32be(const uint8_t* p) {
    return ((uint32_t)p[0]<<24)|((uint32_t)p[1]<<16)|((uint32_t)p[2]<<8)|p[3];
}
uint32_t WC4Globals::r32le(const uint8_t* p) {
    return p[0]|(p[1]<<8)|(p[2]<<16)|((uint32_t)p[3]<<24);
}

bool WC4Globals::load(const uint8_t* data, size_t size) {
    ...
    loaded = true;
    printf("WC4Globals: loaded — %zu paths, %zu fonts, cursors=%s\n",
...
// ---------------------------------------------------------------------------
void WC4Globals::parsePath(const uint8_t* data, size_t size) {
    size_t pos = 0;
    while (pos + 8 <= size) {
        char   tag[5] = {};
...
// ---------------------------------------------------------------------------
void WC4Globals::parseFont(const uint8_t* data, size_t size) {
    size_t pos = 0;
    while (pos + 8 <= size) {
        uint32_t csz = r32be(data + pos + 4);
        size_t   cend = pos + 8 + csz;
        ...
        if (memcmp(data+pos, "FORM", 4) == 0 && csz >= 4 &&
            ...
            while (j + 8 <= cend) {
                ...
                WC4Font* font = new WC4Font();
                if (font->load(data + j + 8, fsz)) {
                    fonts[name] = font;
                } else {
                    delete font;
                    printf("WC4Globals: failed to parse font %s\n", name);
                }
                j = fend + (fsz & 1);
            }
        }
...
// ---------------------------------------------------------------------------
void WC4Globals::parseWrld(const uint8_t* data, size_t size) {
    size_t pos = 0;
    while (pos + 8 <= size) {
        uint32_t csz = r32be(data + pos + 4);
        if (pos + 8 + csz > size) break;
...
// ---------------------------------------------------------------------------
void WC4Globals::parsePntr(const uint8_t* data, size_t size) {
    size_t pos = 0;
    while (pos + 8 <= size) {
        uint32_t csz = r32be(data + pos + 4);
        if (pos + 8 + csz > size) break;
        if (memcmp(data+pos, "SHAP", 4) == 0) {
            // InitFromRAM takes a non-const pointer and owns nothing — safe to cast
            cursorPak.InitFromRAM("globals_cursors",
                                  const_cast<uint8_t*>(data + pos + 8), csz);
            cursorSet = new RSImageSet();
            cursorSet->InitFromPakArchive(&cursorPak);
            printf("WC4Globals: loaded %zu cursor(s)\n", cursorSet->GetNumImages());
            return;
        }
        pos += 8 + csz + (csz & 1);
    }
...
// ---------------------------------------------------------------------------
std::string WC4Globals::getPath(const char* tag) const {
    auto it = paths.find(tag);
    return it != paths.end() ? it->second : "";
}

WC4Font* WC4Globals::getFont(const char* name) const {
