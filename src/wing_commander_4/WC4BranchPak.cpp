...
uint32_t WC4BranchPak::u32le(const uint8_t* p) {
    return p[0] | (p[1]<<8) | (p[2]<<16) | (p[3]<<24);
}
uint32_t WC4BranchPak::u32be(const uint8_t* p) {
...
// ---------------------------------------------------------------------------
bool WC4BranchPak::decompressType40(const uint8_t* src, size_t srcSize,
...
// ---------------------------------------------------------------------------
// Parse a FORM TRUE IFF entry
// ---------------------------------------------------------------------------
bool WC4BranchPak::parseFormTrue(const uint8_t* data, size_t size, WC4BranchEntry& out) {
    if (size < 12 || memcmp(data, "FORM", 4) != 0) return false;
    if (memcmp(data + 8, "TRUE", 4) != 0) return false;
    ...
    return !out.text_eng.empty() || !out.audio.empty();
}

// ---------------------------------------------------------------------------
// Load PAK
// ---------------------------------------------------------------------------
bool WC4BranchPak::load(const uint8_t* data, size_t size) {
    entries.clear();
    ...
    if (advSz != (uint32_t)size) {
        printf("WC4BranchPak: size mismatch advertised=%u actual=%zu\n", advSz, size);
        return false;
    }
    ...
    for (uint32_t i = 0; i < numEntries; i++) {
        ...
        WC4BranchEntry entry;
        if (!parseFormTrue(entryData, entrySize, entry))
            printf("WC4BranchPak: entry %u parse failed (type=%02x sz=%zu)\n", i, type, srcSz);

        entries.push_back(std::move(entry));
    }

    printf("WC4BranchPak: loaded %zu entries\n", entries.size());
    return !entries.empty();
}

const WC4BranchEntry* WC4BranchPak::getEntry(int index) const {
...
// ---------------------------------------------------------------------------
void WC4BranchPak::playEntry(int index) const {
    const WC4BranchEntry* e = getEntry(index);
    if (!e) return;

    if (!e->text_eng.empty())
        printf("Blair: \"%s\"\n", e->text_eng.c_str());

    if (e->audio.empty()) return;
    ...
    if (!dev) {
        printf("WC4BranchPak: SDL_OpenAudioDevice failed: %s\n", SDL_GetError());
        return;
    }

    SDL_QueueAudio(dev, e->audio.data(), (uint32_t)e->audio.size());
